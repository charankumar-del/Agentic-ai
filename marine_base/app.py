...
import time
import uuid
import agent # Imports the brain from agent.py
    ...
    if uploaded_file:
        ...
        if st.button("🔍 Run Full Inspection"):
            ...
            try:
                ...
                insert_inspection({
                    "inspection_id": inspection_id, "file_name": uploaded_file.name,
                    "detected_classes": detected_classes, "highest_confidence": float(max_conf),
                    "risk_level": risk, "inference_time": float(inference_time),
                    "precision": MODEL_PRECISION, "recall": MODEL_RECALL,
                    "map50": MODEL_MAP50, "map5095": MODEL_MAP5095,
                    "image_url": img_url, "annotated_image_url": ann_url,
                    "pdf_url": rep_url, "status": "completed"
                })
...
if mode == "Admin":
    ...
    # ... Rest of your Admin Login/Dashboard logic stays the same ...
